...
The training page includes a pre-workout joint check with pain-aware guidance. The weekly report and Week 1 comparisons use recorded evidence, with unknown data shown as unknown. Photo records do not count as recovery check-ins or generate active-day streaks. Nutrition on Today saves optional meals, protein, produce, and optional calories in the version 9 IndexedDB table; targets are individually configurable and never used for penalty XP. This is a progression layer; the original workout, schedule, Bae Time, hydration, rewards, and persistence logic remain authoritative.

Install test dependencies with `pnpm install --frozen-lockfile`; run `pnpm test` (Node.js 24+). Production browser code has no package dependencies. All 125 regression tests pass.
...
Profile contains Export Backup / Import Backup. Database schema is version 9, with append-only migrations and matching backup adapters. Never edit a released schema migration; append a new version. Legacy app migration remains deliberately deferred.
